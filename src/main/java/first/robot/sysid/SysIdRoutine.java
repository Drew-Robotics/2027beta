 ...
 *
 * <p>A single mechanism may have multiple sub-mechanisms, but they should not share test routines.
 * Each complete test of a mechanism should have its own SysIdRoutine instance, since the log name
 * of the recorded data is determined by the mechanism name.
 *
...
public class SysIdRoutine extends SysIdRoutineLog {
  ...
  public static class Mechanism {
    ...
    // Named for what it does rather than for its type, and qualified, because this class shadows
    // the command framework's Mechanism by its own name.
    public final org.wpilib.command3.Mechanism owner;
    ...
    public Mechanism(
        ...
        org.wpilib.command3.Mechanism mechanism,
        String name) {
      this.drive = drive;
      this.log = log != null ? log : l -> {};
      this.owner = mechanism;
      this.name = name != null ? name : mechanism.getName();
    }

    /**
     * Create a new mechanism specification for a SysId routine. Defaults the log name to the
     * mechanism name.
     *
     * @param drive Sends the SysId-specified drive signal to the mechanism motors during test
     *     routines.
   ...
   * per iteration. Upon command cancellation, the `drive` callback is called with a value of 0
   * volts.
   *
  ...
  public Command quasistatic(Direction direction) {
    ...
    return m_mechanism
        ...
        // The timeout below is a race, so this body is always cancelled and never ends: after the
        // loop is a line that never runs.
        .whenCanceled(
            () -> {
